...
class Solution:
    ...
    def x_dict(self) -> dict:
        """Return {variable_name: value} for the optimal solution.

        Uses variable names when available, falling back to index as key.
        """
        return {
            v.name if v.name else str(v.index): float(self.x[i])
            for i, v in enumerate(self.problem._variables)
        }
    ...
    **options,
    ...
    def _apply_offset(obj_val: float, status: str) -> float:
        ...
        if status in ("solved", "optimal", "OPT"):
            return obj_sign * float(obj_val) + obj_sign * obj_offset
        return obj_sign * float(obj_val)

    def _filter_y(y_arr: np.ndarray, status: str) -> np.ndarray:
        """Return y only for successful solutions; zeros otherwise."""
    ...
    if solver == "osqp":
        ...
        return Solution(
            ...
            solver=solver,
            problem=problem,
            y=_filter_y(y_arr, result["status"]),
