...
def supernodal_solver_cls():
    ...
    _project_root = _test_dir.parents[2]  # python/tests/extensions/.. = project root
    _so = _project_root / "build" / "supernodal.cpython-314-x86_64-linux-gnu.so"
    if not _so.exists():
        # Try common suffixes
        _so = next((_project_root / "build").glob("supernodal.*.so"), None)
    if _so is None:
