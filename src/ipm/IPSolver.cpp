...
IPSolver::convertToSparseDiagonal(const Eigen::VectorXd &vec) {
...
void IPSolver::solve_newton_system(
    ...
    const Eigen::VectorXd &delta_y, const Eigen::VectorXd &delta_z,
 ...
 *
 */
void IPSolver::solve(const Eigen::SparseMatrix<double> &A,
...
void IPSolver::run_optimization(const OptimizationData &data, const double tol) {
  ...
  for (int k = 0; k < max_iter; ++k) {
    ...
    while (ncor <= 2 && alpha < 0.9995) {
      ...
      solve_newton_system(Delta_x_c, Delta_lambda_c, Delta_w_c, Delta_s_c,
                          Delta_v_c, Delta_tau_c, Delta_kappa_c, ls, theta_vw,
                          b, c, ubi, ubv, delta_x, delta_y, delta_z, delta_0, x,
                          lambda, w, s, v, tau, kappa, zero_rp, zero_ru,
                          zero_rd, 0, -t_xs, -t_vw, -t0);
...
OptimizationData
IPSolver::convertToOptimizationData(const ModelData &modelData) {
...
Eigen::SparseMatrix<double>
ModelData::SparseMatrix::toEigenSparseMatrix() const {
  Eigen::SparseMatrix<double> mat(num_rows, num_cols);
  std::vector<Eigen::Triplet<double>> triplets;
  triplets.reserve(values.size());
  for (int i = 0; i < num_rows; ++i) {
    for (int j = rowPtrs[i]; j < rowPtrs[i + 1]; ++j) {
      triplets.emplace_back(i, colIndices[j], values[j]);
    }
  }
