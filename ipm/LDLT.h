 ...
 *
 * This file declares the LDLT interfaces and helper functions used by the
 * BALDES solver.
 *
 */
...
public:
  ...
  CustomSimplicialLDLT()
      : m_isInitialized(false), m_info(Success), m_P(0), m_Pinv(0),
        m_matrix(1, 1), m_L(m_matrix), m_U(m_matrix.adjoint()), m_epsilon(1e-9),
  ...
  void factorize_preordered(const CholMatrixType &ap) {
    ...
    // Sequential factorization of each column.
    ...
    for (StorageIndex k = 0; k < size; ++k) {
      ...
      for (StorageIndex i : pattern) {
        ...
        if (yi != Scalar(0)) {
          ...
          // Update entries in the local buffer.
  ...
  template <int SrcMode_, int DstMode_, bool NonHermitian, typename MatrixType,
            int DstOrder>
  void
  permute_symm_to_symm(const MatrixType &mat,
    ...
    enum {
      ...
      DstMode =
          DstOrder == RowMajor ? (DstMode_ == Upper ? Lower : Upper) : DstMode_,
      SrcMode =
          SrcOrder == RowMajor ? (SrcMode_ == Upper ? Lower : Upper) : SrcMode_
    };
  ...
  Eigen::VectorXd solve(const MatrixBase<Rhs> &b) const {
    ...
    // Regular solve path
    dest = matrixL().solve(dest);
    dest.array() /= m_diag.array();
    dest = matrixU().solve(dest);
