 ...
 *
 * This file declares the Evaluator interfaces and helper functions used by the
 * BALDES solver.
 *
 */
...
template <typename MatrixType> class CustomMatIterator {
public:
  ...
  EIGEN_STRONG_INLINE CustomMatIterator(const MatrixType &mat, StorageIndex col)
      : matrix_(mat), outerIndex_(matrix_.outerIndexPtr()[col]),
              ...
              if (!(Mode & Eigen::UnitDiag)) {
                ...
                const Scalar reg_val =
                    std::abs(diag_val) < regularizationFactor
                        ? (diag_val >= 0 ? regularizationFactor
